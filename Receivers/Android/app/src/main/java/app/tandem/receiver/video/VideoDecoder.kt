...
// there; the Choreographer pacing, parameter-set injection and the Tandem frame
// contract are new.
...
/**
 * MediaCodec decode of the Tandem VIDEO stream onto a [Surface].
 *
...
class VideoDecoder(
    ...
    private fun startVsyncPacing() {
        val thread =
            HandlerThread("tandem-vsync", Process.THREAD_PRIORITY_DISPLAY).apply { start() }
        vsyncThread = thread
    ...
    private fun setupDecoder() {
        val thread =
            HandlerThread("tandem-decoder", Process.THREAD_PRIORITY_DISPLAY).apply { start() }
        codecThread = thread
        val handler = Handler(thread.looper)
    ...
    private companion object {
        const val TAG = "TandemVideoDecoder"
