...
import app.teras.receiver.input.InputRouter
import app.teras.receiver.input.InputSink
import app.teras.receiver.input.VideoViewport
...
class DisplayView(context: Context) : FrameLayout(context) {
    ...
    private val router =
        InputRouter(
            sink =
                object : InputSink {
                    override fun touch(phase: Byte, pointers: List<app.teras.receiver.net.TouchPointer>) =
                    ...
                    override fun key(event: app.teras.receiver.net.KeyEvent) =
                        sink?.key(event) ?: Unit
                },
