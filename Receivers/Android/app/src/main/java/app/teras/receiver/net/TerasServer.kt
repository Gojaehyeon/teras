...
class TerasServer(
    ...
    @Volatile private var current: TerasConnection? = null

    /** Called on the accept thread when a session starts or ends. */
    var onSessionStarted: ((TerasConnection) -> Unit)? = null
    var onSessionEnded: ((TerasConnection, String) -> Unit)? = null
    var onListenFailed: ((Throwable) -> Unit)? = null

    val activeConnection: TerasConnection? get() = current
    ...
    fun start() {
        ...
        serverSocket = server
        acceptThread =
            Thread({ acceptLoop(server) }, "teras-accept").apply {
                isDaemon = true
                start()
            }
