...
final class CryptoTests: XCTestCase {
    ...
    func testPairFlow() throws {
        let pinKey = TerasCrypto.pinKey(pin: "123456", deviceId: deviceId, hostId: hostId)
        let proof = TerasCrypto.pairProof(pinKey: pinKey, hostNonce: hostNonce, deviceNonce: deviceNonce)
        XCTAssertEqual(proof.count, 32)
        let box = try TerasCrypto.sealPairBox(secret: secret, pinKey: pinKey, hostNonce: hostNonce, deviceNonce: deviceNonce)
        XCTAssertEqual(box.count, 48)
        XCTAssertEqual(try TerasCrypto.openPairBox(box, pinKey: pinKey, hostNonce: hostNonce, deviceNonce: deviceNonce), secret)
        XCTAssertThrowsError(try TerasCrypto.openPairBox(box, pinKey: TerasCrypto.pinKey(pin: "000000", deviceId: deviceId, hostId: hostId), hostNonce: hostNonce, deviceNonce: deviceNonce))
    }

    func testSessionCipherRoundTripAndReplay() throws {
        let keys = TerasCrypto.sessionKeys(secret: secret, hostNonce: hostNonce, deviceNonce: deviceNonce)
    ...
    func testPrintVectors() throws {
        ...
        let lines = [
            "pinKey \(pinKey.hex)",
            "pairProof \(TerasCrypto.pairProof(pinKey: pinKey, hostNonce: hostNonce, deviceNonce: deviceNonce).hex)",
            "authProof \(TerasCrypto.authProof(secret: secret, hostNonce: hostNonce, deviceNonce: deviceNonce).hex)",
            "authAckProof \(TerasCrypto.authAckProof(secret: secret, hostNonce: hostNonce, deviceNonce: deviceNonce).hex)",
            "pairBox \(box.hex)",
