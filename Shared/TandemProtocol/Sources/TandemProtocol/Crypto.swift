...
public enum TandemCrypto {
    ...
    /// PAIR_OK box = AES‑256‑GCM(key = HKDF(pinKey, salt, "tandem-v1-pairbox"), nonce = 12 zero bytes, secret) → ciphertext ‖ tag
    public static func sealPairBox(secret: Data, pinKey: Data, hostNonce: Data, deviceNonce: Data) throws -> Data {
        let key = hkdf(pinKey, salt: hostNonce + deviceNonce, info: "tandem-v1-pairbox", count: 32)
    ...
    public static func sessionKeys(secret: Data, hostNonce: Data, deviceNonce: Data) -> (h2r: Data, r2h: Data) {
        ...
        return (hkdf(secret, salt: salt, info: "tandem-v1-h2r", count: 32),
                hkdf(secret, salt: salt, info: "tandem-v1-r2h", count: 32))
    }
